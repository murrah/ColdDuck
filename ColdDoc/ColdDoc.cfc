...
<!--- MH: None of your edits are necessary for ColdDuck. 
Just lots of debug output while you worked this out --->
...
<cffunction name="generate" hint="generates the documentation" access="public" returntype="void" output="true">
...
//WriteOUtput("<br>Path="&currentPath);
                if(len(currentPath))
                {
...
//WriteOUtput("<br>packagePath="&packagePath&", cfcName="&cfcName);

				try
				{
					//MH NB: CF getComponentMetaData() throws an error 
					//if the 'DisplayName' attribute is missing from the component tag

	                if (Len(packagePath)) {
	                   	//WriteOutput("<br>Getting metadata for "&packagePath & "." & cfcName);
	                    meta = getComponentMetaData(packagePath & "." & cfcName);
	                }
	                else {
	                	//WriteOutput("<br>Getting metadata for " & cfcName);
...
<cffunction name="warnError" hint="Warn the user that there was an error through cftrace" access="private" returntype="void" output="false">
	...
	<cfset _trace(error)>
	 ...
	 <!--- MH: --->
	 <cfset errMsg = toString(arguments.error) />
	 <cfif findNoCase("coldfusion.compiler.TokenMgrError",errMsg)>
		<cfset errMsg = errMsg & "<br>Possibly missing 'DisplayName' attribute from component tag. CF getComponentMetaData() throws this error if DisplayName is missing.">
	</cfif>
	 <cfoutput><br><br>Warning, the following script has errors: #arguments.cfcName#, #errMsg#</cfoutput>
	<cfdump var="#arguments#" label="warnError">
	<cfabort><!---  --->
