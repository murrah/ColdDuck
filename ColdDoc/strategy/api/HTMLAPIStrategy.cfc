...
<cffunction name="writePackagePages" hint="writes the package summaries" access="private" returntype="void" output="false">
	...
	<cfoutput query="arguments.qMetaData" group="package">
		<cfscript>
			...
			// MH Improvement for ColdDoc. 
			// Add <br> to the hint lines so it reads formatted in the output docs
			local.tmp = qClasses.metadata;
			//writeDump(tmp.functions);			
			for (local.i=1; i lte arrayLen(tmp.functions); i=i+1) {
				//writeOutput("i="&i);
				if (structKeyExists(tmp.functions[i],'hint')){
					local.hint = tmp.functions[i].hint;
					local.htmlHint = replace(local.hint,"#Chr(10)#","<br/>","All");
					//writeOutput(htmlHint);			
					local.tmp.functions[i].hint = local.htmlHint;
				} else {
					// local.tmp.functions[i].hint = "No hint";
				}
				
			}
			
			local.tmp = qPackage.metadata;
			//writeDump(tmp.functions);			
			for (local.i=1; i lte arrayLen(tmp.functions); i=i+1) {
				//writeOutput("i="&i);
				if (structKeyExists(tmp.functions[i],'hint')){
